		...
		if (Math.abs(target) - Math.abs(dToMaxVel) - Math.abs(dToVf) < 0) {
			if (vi == 0 && vf == 0) {
				...
				if (d <= target) {
					...
					/*
					 * This is a problem. We can't go from vi to vf without overshooting our
					 * distance while obeying maxAccel so we need to rescale our acceleration to
					 * allow for this
					 * 
					 * This should be avoided since the result may be unachievable by the robot, but
					 * it's better than sitting like a brick
					 */
